...
async function sendPanel(req, res, next) {
  try {
    ...
    try {
      const info = await mail.sendPanelLink({ to, url, projectName: project.name, clientName: project.client ? project.client.name : null, client: project.client || null });
      await events.emailSent({ kind: 'Panel projektu', to, info, projectId: project.id, ip: req.ip });
