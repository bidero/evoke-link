...
function overlayHtml(bg) {
  ...
  // UWAGA: odsłonięcie jest NATYCHMIASTOWE (bez transition) — animowana opacity na warstwie
  // fixed pod backdrop-filter zostawiała na niektórych GPU nieświeżą próbkę tła w obszarze
  ...
  if (b.type === 'image' && img && !slideshowActive) {
    ...
    html += `<div aria-hidden="true" class="bg-img-layer" style="position:${pos};inset:0;z-index:0;pointer-events:none;opacity:0;">${inner}</div>`;
    html += preloader();
  }
