// Minimalistyczne ikony w stylu Lucide (https://lucide.dev) jako inline SVG.
// Kontur, bez wypełnienia, stroke = currentColor (kolor sterujemy klasą, domyślnie brand).
// Użycie w widoku:  <%- icon('download') %>  lub  <%- icon('upload', { cls: 'text-red-500', size: 20 }) %>
...
const PATHS = {
  ...
  // Ptaszki statusu wiadomości (jak w komunikatorach): tick = wysłane, checkCheck = przeczytane.
...
function icon(name, { size = 18, cls = 'text-brand-600', strokeWidth = 2 } = {}) {
  const body = PATHS[name] || '<circle cx="12" cy="12" r="2"/>';
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" ` +
    `fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" ` +
    `stroke-linejoin="round" class="inline-block align-[-0.125em] ${cls}">${body}</svg>`
  );
}

// Mapowanie typu zdarzenia na ikonę (używane w historii, powiadomieniach, dashboardzie).
const EVENT_ICON = {
  created: 'plus',
  downloaded: 'download',
  uploaded: 'upload',
