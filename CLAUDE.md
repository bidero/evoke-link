...
- **Event** — historia + powiadomienia + dane widżetów. `type` (created|downloaded|uploaded|viewed|email_sent|note|updated|expired|error|onboarded), `projectId?`, `transferId?`, `clientId?` (oś czasu klienta), `isRead`, `dismissed`, `meta` (JSON string). Powiadomienia (dzwonek) = tylko `NOTIFY_TYPES` (uploaded|downloaded|error|approved|changes|onboarded); `viewed` jest w osi czasu, ale NIE dzwoni. Klik w powiadomienie: transfer → projekt → klient (fallbacki w `notification.controller.open`).
