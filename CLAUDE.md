...
- **Event** — historia + powiadomienia + dane widżetów. `type` (created|downloaded|uploaded|viewed|email_sent|note|updated|expired|error|onboarded), `projectId?`, `transferId?`, `clientId?` (oś czasu klienta), `isRead`, `dismissed`, `meta` (JSON string). Powiadomienia (dzwonek) = tylko `NOTIFY_TYPES` (uploaded|downloaded|error|approved|changes|onboarded|paid_declared); `viewed` jest w osi czasu, ale NIE dzwoni. Klik w powiadomienie: transfer → projekt → klient (fallbacki w `notification.controller.open`).
...
- [ ] Do zrobienia: backup off-site; oferty z akceptacją w portalu; załączniki w wiadomościach; drugi user/role. UWAGA: white-label per-klient/projekt POZA ZAKRESEM NA STAŁE (decyzja 2026-07-02) — nie proponować.
